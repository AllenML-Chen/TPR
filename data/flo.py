    ...
    def __init__(self, root, split='train',
        ...
        if self.split == "train":
            with open(self.root + "/trainval_img_id.txt", "r") as f:
                for line in f:
                    img_id = line.strip()
                    img_path = os.path.join(self.root, self.images_path[img_id])
        ...
        self.CLS2TXT = {key: value for key, value in zip(CLASS_NAME['flo'], TEXT['flo'])}
        self.class_texts = [self.CLS2TXT[cls] for cls in self.classes]
        
        with open(self.root + "/trainval_img_id.txt", "r") as f:
            seen_labels = [img_label[line.strip()] for line in f] 
        self.seen_idx = np.array([1 if str(i+1) in seen_labels else 0 for i in range(102)])
...
if __name__ == '__main__':
    ...
    train_dataset = FLO(root, split='train')
    print(len(train_dataset))
    print(np.unique(train_dataset._labels).shape)
    print(np.unique(train_dataset._labels))
    test_seen_dataset = FLO(root, split='test_seen')
    print(len(test_seen_dataset))
    print(np.unique(test_seen_dataset._labels).shape)
    test_unseen_dataset = FLO(root, split='test_unseen')
    print(len(test_unseen_dataset))
    print(np.unique(test_unseen_dataset._labels).shape)
    print(np.unique(test_unseen_dataset._labels))
    print(set(train_dataset._labels)&set(test_unseen_dataset._labels))