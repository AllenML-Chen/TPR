...
class EuroSAT(Dataset):
    ...
    def __init__(self, root, split='train',
        ...
        self.CLS2TXT = {key: value for key, value in zip(CLASS_NAME['eurosat'], TEXT['eurosat'])}
        self.class_texts = [self.CLS2TXT[cls] for cls in self.classes]
        
        with open(self.root + "/trainval_img_id.txt", "r") as f:
            seen_labels = [img_label[line.strip()] for line in f] 
        self.seen_idx = np.array([1 if str(i+1) in seen_labels else 0 for i in range(10)])
...
if __name__ == '__main__':
    ...
    test_seen_dataset = EuroSAT(root, split='test_seen')
    print(len(test_seen_dataset))
    test_unseen_dataset = EuroSAT(root, split='test_unseen')
