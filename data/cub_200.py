    ...
    def __init__(self, root, split='train',
        ...
        with open(os.path.join(self.root, 'classes.txt')) as f:
            for line in f:
                ...
                class_name = ' '.join(class_name.split('.')[1].split('_'))
        ...
        self.CLS2TXT = {key: value for key, value in zip(CLASS_NAME['cub_200'], TEXT['cub_200'])}
        self.class_texts = [self.CLS2TXT[cls] for cls in self.classes]
        
        with open(self.root + "/trainval_img_id.txt", "r") as f:
            seen_labels = [img_label[line.strip()] for line in f] 
        self.seen_idx = np.array([1 if str(i+1) in seen_labels else 0 for i in range(200)])
...
if __name__ == '__main__':
    ...
    root = '../datasets/CUB_200_2011'
    train_dataset = CUB(root, split='train')
    print(len(train_dataset))
    print(np.unique(train_dataset._labels))
    test_seen_dataset = CUB(root, split='test_seen')
    print(len(test_seen_dataset))
    test_unseen_dataset = CUB(root, split='test_unseen')
