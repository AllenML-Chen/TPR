...
def calibrate(sf, uf, y_true_st, y_true_ut, idx):

    sf, uf = sf.cpu().numpy(), uf.cpu().numpy()
    y_true_st, y_true_ut = np.array(y_true_st), np.array(y_true_ut)
    ...
    txt = ('(SEEN) %.4f\t(UNSEEN) %.4f\t (HM) %.4f\t(Gamma): %.2f' %
           (acc_seen[idx] * 100,
            acc_unseen[idx] * 100,
            acc_hm[idx] * 100,
            gammas[idx]))
